...
if(isset($_POST["update"])) {
    ...
    $reply      = $_POST["reply"];
    $status     = $_POST["status"];
    mysqli_query($connect, "UPDATE comment SET post_id = '$post_id', name = '$name', reply = '$reply',
                            status = '$status' WHERE id = '$id_comment'");
    header("location:index.php?comment");
}

/** TAMPILKAN DATA PADA FORM **/
$comment_id     = $_GET["comment-update"];
$edit           = mysqli_query($connect, "SELECT * FROM comment WHERE id = '$comment_id'");
$row_edit       = mysqli_fetch_array($edit);



$post           = mysqli_query($connect, "SELECT * FROM post ORDER BY id ASC");

$comment        = mysqli_query($connect, "SELECT comment.*, post.title
                                          FROM comment, post
