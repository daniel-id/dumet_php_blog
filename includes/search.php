...
if (isset($_GET["page-search"])) {
  $current_page   = $_GET["page-search"];
  $current_page   = ($current_page > 1) ? $current_page : 1;
}
$total_data       = mysqli_num_rows(mysqli_query($connect, "SELECT * FROM post WHERE title LIKE '%$keyword%'"));
    ...
    <nav class="text-center">
      <ul class="pagination">
        <?php if($current_page > 1) { ?>
          <li>
              <a href="index.php?page-search=<?php echo $current_page - 1 ?>" aria-label="Previous">
        ...
        <?php if($current_page < $total_page) { ?>
          <li>
            <a href="index.php?page-search=<?php echo $current_page + 1 ?>" aria-label="Next">
